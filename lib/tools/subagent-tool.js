...
export function composeReusableThreadId(reuseKey) {
  return reuseKey ? `reusable::${reuseKey}` : null;
}
...
export function createSubagentTool(deps) {
  ...
  return {
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      // access 仅取合法枚举，非法值按省略处理（继承父档）。后缀(instance)与 access 正交，不参与判档。
      ...
      const threadKind = reuseKey ? "reusable" : "ephemeral";
      const threadId = reuseKey ? composeReusableThreadId(reuseKey) : taskId;
