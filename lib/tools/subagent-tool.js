...
/**
 * 复用实例稳定身份：realAgentId + 任务后缀（如 毛毛::探索）。
 * 红线：taskSuffix 仅参与此 key 与展示，绝不进 resolveAgentParam（否则把后缀当 agent 解析失败）。
 */
export function composeReuseKey(agentId, taskSuffix) {
...
export function createSubagentTool(deps) {
  ...
  return {
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      const reuseKey = (taskSuffix && reuseStore) ? composeReuseKey(realAgentId, taskSuffix) : null;
