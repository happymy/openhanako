...
function getBundledShellCandidates(env = process.env, deps = {}) {
  ...
  const found = [];
  const gitRoots = [
    resourcesPath ? join(resourcesPath, "git") : null,
  ...
  for (const gitRoot of gitRoots) {
    ...
    const binDir = joinPath(gitRoot, "mingw64", "bin");
    ...
    for (const candidate of shellCandidates) {
      const shell = joinPath(binDir, candidate.exe);
...
function getShellEnvForCandidate(baseEnv, shellInfo) {
  ...
  const isWinPath = /^[a-z]:[\\/]|^\\\\/i.test(shellInfo?.shell || "");
  const delimiter = current.includes(";") || isWinPath ? ";" : path.delimiter;
  const joinPath = isWinPath ? path.win32.join : join;
  const dirnamePath = isWinPath ? path.win32.dirname : dirname;
  const dirs = [];
  if (shellInfo?.shell) dirs.push(dirnamePath(shellInfo.shell));
  if (shellInfo?.bundledRoot) dirs.push(joinPath(shellInfo.bundledRoot, "cmd"));
