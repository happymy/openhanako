 ...
 * 例外：workflow / workflow_agent 通过可选的 store 做持久化背书（重启不丢右侧 WorkflowCard）。
 */
...
// 只有 workflow / workflow_agent 走持久化背书。subagent 有自己的 SubagentRunStore + 历史块；
// heartbeat / cron 本就瞬时，不持久化。
const PERSISTABLE_KINDS = new Set(["workflow", "workflow_agent"]);
...
function normalizeEntry(entry, existing) {
  return {
    ...
    // subagent 复用实例后缀（区分同名实例，如 毛毛·探索）；非复用 subagent / 其它 kind 为 null。
    reuseInstance: pickStr(entry.reuseInstance, existing?.reuseInstance ?? null),
