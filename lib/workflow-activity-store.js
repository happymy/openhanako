/**
 * workflow-activity-store.js —— 右侧 workflow 卡的持久化背书
 *
 * ActivityHub 是内存广播层，进程重启即清空，导致右侧 WorkflowCard 重启消失。
 * 这个 store 把 workflow / workflow_agent 活动落盘（hanakoHome/workflow-activity.json），
 * 作为 ActivityHub 的「持久化背书」：upsert 写穿、重启回灌、会话退场清理、72h TTL 修剪。
 *
